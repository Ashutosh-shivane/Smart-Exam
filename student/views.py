    ...
    if request.COOKIES.get('course_id') is not None:
        ...
        for i in range(len(questions)):
            ...
            if selected_ans == actual_answer:
                ...
                if course.negative_marking:
                    total_marks -= (questions[i].marks*course.negative_marking_percentage/100)
...
def view_result_view(request):
    ...
    student = request.user.student  # Assuming a OneToOne relationship with User
    courses = QMODEL.Course.objects.filter(result__student=student).distinct()
...
def check_test(request,pk):
    ...
    course = QMODEL.Course.objects.get(id=pk)
    print(course)
    return render(request,"student/studentchec.html",context={"pk":pk,"coursename":course})
...
def check(request,pk):
    ...
    if excel_pass_check(pk,p) == True :
        ...
        messages.error(request, "Invalid exam password. Please try again.")
        return redirect('check-exam', pk=pk)
